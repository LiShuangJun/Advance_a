...
layui.use(['layim', 'laytpl'], function(layim) {
  ...
  socket.onopen = function() {
    // 登录
    var login_data = '{"type":"init","id":"' + uid + '", "username":"' + uname +
      '", "avatar":"' + avatar + '", "sign":"' + sign + '"}';
              ...
              function(res) {
                if (1 == res.code) {
                  ...
                  var join_data = '{"type":"joinGroup", "join_id":"' + data.data.joinid +
                    '"' +
                    ', "group_id": "' + data.data.groupid + '", "group_avatar": "' + data
                    .data.groupavatar + '"' +
  ...
  my_events = {
    ...
    enterZone: function(othis, e) {
      ...
      layer.alert('<a href="/index/Userzone/uZone/fid/' + friend_id +
        '"  target="_blank">点击，进入他的空间</a>', {
          title: false,
          icon: 6,
          btn: ''
        });
      ...
      layer.confirm('确定要将他加入黑名单？', {
        ...
        $.getJSON("/index/Tools/joinBlack/fid/" + friend_id, function(res) {
          if (1 == res.code) {
            ...
            //通知被加入黑名单的用户，删除我
            var black_data = '{"type":"black","to_id":"' + res.data.to_id +
    ...
    changeGroup: function(othis, e) {
      ...
      var elemAddTpl = ['<div class="layim-add-box">',
        ...
        '{{ d.data.name||"" }}</p></div>', '<div class="layim-add-remark">',
        '{{# if(d.data.type === "friend" && d.type === "setGroup"){ }}', '<p>选择分组</p>',
        '{{# } if(d.data.type === "friend"){ }}',
        '<select class="layui-select" id="LAY_layimGroup">',
        '{{# layui.each(d.data.group, function(index, item){ }}',
        '<option value="{{ item.id }}">{{ item.groupname }}</option>', '{{# }); }}',
        '</select>', '{{# } }}', '{{# if(d.data.type === "group"){ }}', '<p>请输入验证信息</p>',
        '{{# } if(d.type !== "setGroup"){ }}',
      ...
      layer.confirm('确定删除该好友？', {
        ...
        $.post('/index/Tools/removeFriend', {
          ...
          if (1 == res.code) {
            ...
            //通知被删除的用户，删除我
            var black_data = '{"type":"delFriend","to_id":"' + res.data.to_id +
              '", "del_id":"' + res.data.del_id + '"}';
            socket.send(black_data);
          } else {
            layer.msg(res.msg, {
      ...
      var index = layer.confirm('确定退出该群组？', {
        ...
        $.post('/index/Tools/leaveGroup', {
          ...
          if (1 == res.code) {
            ...
            // 退出讨论组
            var leave_data = '{"type": "leaveGroup", "leave_id":"' + res.data.uid +
