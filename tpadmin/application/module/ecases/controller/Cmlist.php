...
class Cmlist extends Base
{
    ...
    {
        $this->siteTitle = 'Casemanager recommended list';
    ...
    protected function _page($model, $rowNum = null, \Closure $perform = null)
    {
      ...
      $model = $this->buildModel($model);
        ...
        if(empty($casedata)){
            $this->error('No valid information was found for this id', module_url('case_list/index'));
            exit;
        }
...
//            $this->redirect(module_url('case_list/index'));
        ...
        foreach ($list as $key => $value) {
            ...
            foreach($value->langarr as $k=>$v){
                if($v['id']==$language){
                   $langname[$v['id']]='<span style="color:green;">'.$v['l_ename'].'</span>'; 
                   $list[$key]['tuijian']=1;
                }else{
                   $langname[$v['id']]=$v['l_ename']; 
            ...
            foreach($value->ksarr as $k=>$v){
                if($v['ks_id']==$casedata['ks_type']){
                   $ksname[$v['ks_id']]='<span style="color:green;">'.$v['ks_ename'].'</span>'; 
                   $list[$key]['tuijian']=1;
                }else{
                   $ksname[$v['ks_id']]=$v['ks_ename']; 
