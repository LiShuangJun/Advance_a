...
use core\manage\validate\EuserValidate;
...
class User extends Base
{
    ...
    {
        $this->siteTitle = 'Administrator management';
    ...
    public function add(Request $request)
    {
        if ($request->isPost()) {
            ...
            // 验证
            $this->_validate(EuserValidate::class, $data, 'add');
            ...
            $this->siteTitle = 'Add administrator';
    ...
    public function edit(Request $request)
    {
        if ($request->isPost()) {
            ...
            $this->_validate(EuserValidate::class, $data, $scene);
            ...
            $this->siteTitle = 'Edit administrator';
    ...
    public function delete()
    {
        ...
        if (UserLogic::getInstance()->isSuperAdmin($userId)) {
            $this->error('Super administrator can not be deleted!');
        } elseif ($this->userId == $userId) {
            $this->error('You can not delete yourself');
        }
    ...
    protected function user_delete($model, $isSoft = false, $url = null)
    {
        ...
        if ($status) {
            $this->success('Successfully deleted', $url);
        } else {
    ...
    {
        $selectUserStatus = UserLogic::getSingleton()->getSelectStatus(2);
