...
use core\cases\validate\EchatUserValidate;
...
class SUserList extends Base
{
    ...
    protected function getUserStatus(){
         ...
         $logic =ChatUserLogic::getInstance();
         $case_manager=$logic->getSelectStatus(2);
    ...
    public function add(Request $request)
    {
        if ($request->isPost()) {
            ...
            $managerid=$request->param('managerid');
              // 验证
            $this->_validate(EchatUserValidate::class, $data, 'add');
          //检测用户名重复
           $where=[
               ...
               [
                   ...
                   'msg'=>'The administrator has opened a layim account.'
               ]
  ...
  public function getLangList() {
      $list=ChatUserLogic::getInstance()->getLanguageList(2);
    ...
    {
        $userid=$this->_id();
        $userid || $this->error('Illegal operation');
        if ($request->isPost()) {
            $data = [
                'user_name' => $request->param('user_name'),
                'sex' => $request->param('sex'),
           ...
           if($request->param('pwd')){
              ...
              $data['pwd_again']=$request->param('pwd_again');
                 // 验证
            $this->_validate(EchatUserValidate::class, $data, 'edit_password');
            //发送邮箱
            $email=new SendUser();
            $email->editSend($data);
            //加密密码
            ...
            unset($data['pwd_again']);
              ...
              // 验证
            $this->_validate(EchatUserValidate::class, $data, 'edit_info');
          }
          //检测用户名重复
           $where=[
    ...
    public function resetpwd(Request $request) {
        ...
        if(empty($users)){
             return json(['code'=>0,'msg'=>'The user status is abnormal or does not exist!']);
        }
        if(empty($users['email'])){
            return json(['code'=>0,'msg'=>'The user mail is empty, please manually set the password!']);
        }
        ...
        if($result){
            ...
            return json(['code'=>1,'msg'=>'Reset successfully! The password has been sent to the user\'s mailbox!']);
        }
