...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  LAYIM.prototype.chat = function(data){
    ...
    //获取当前聊天窗口信息对象
  LAYIM.prototype.getChat = function(data){
    return thisChat().elem.find('.layim-chat-main');
  };
  //打开一个自定义面板
  LAYIM.prototype.panel = function(options){
    return popPanel(options,1,true);
  };
  ...
  var comTpl = function(tpl, anim, back){
      ...
      if(back){
          if(back=='jump'){
               backstr='<i class="layui-icon layim-chat-back" layim-event="jumpback">&#xe603;</i>';
          }else{
              backstr='<i class="layui-icon layim-chat-back" layim-event="back">&#xe603;</i>';
          }
      }
  ...
  //弹出公共面板
  var popPanel = function(options, anim,back){
    options = options || {};
    ...
    return layer.open({
      ...
      ,shadeClose: false
      ,anim: -1
      ,content: laytpl(comTpl(options.tpl, anim === -1 ? false : true, back)).render(data)
      ,success: function(elem){
        var othis = $(elem);
  ...
  var messageNew = {}, getMessage = function(data){
    ...
    if(data.group_fromid == window.mineid){   //新加
      data.mine = true;
    }
...
//  };
  ...
  var viewChatlog = function(){
      if(!window.logdata){
              var local = layui.data('layim-mobile')[cache.mine.id] || {};
              var chatlog = local.chatlog || {};
              var logdatas=chatlog[thatChat.data.type + thatChat.data.id];
      }else{
          var logdatas=window.logdata;
      }
   var thatChat = thisChat();
    var ul = thatChat.elem.find('.layim-chat-main ul');
    layui.each(logdatas, function(index, item){
      if(new Date().getTime() > item.timestamp && item.timestamp - (sendMessage.time||0) > 60*1000){
        ul.append('<li class="layim-chat-system"><span>'+ layui.data.date(item.timestamp) +'</span></li>');
        sendMessage.time = item.timestamp;
      }
